...
- **Cursor:** root skills and the command surface. Routing inherits unless the
  installed version proves compatible optional custom-agent model fields;
  effective routing may remain unverified.
- **Kimi Code:** native skills; routing uses only `primary`, a verified
  experimental `secondary`, or inheritance. Vendor model IDs never enter the
  manifest.
