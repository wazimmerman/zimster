...
| `CTRL-BUDGET-EPOCH-001` | semantic assurance plus lifecycle-authenticated execution-budget scopes | Completion fixture with three historical rechecks across three semantic epochs passes; two rechecks in one epoch or an unreconciled aggregate fails | Legacy aggregate counts remain visible and are accepted only when durable lifecycle attempts reconcile them |
