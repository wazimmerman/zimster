...
- `CTRL-EXEC-001`: Count broad suites and exact duplicate commands only through governed executions with durable pre-execution identity, compact outcomes, receipts, events, and candidate/runtime provenance; every admitted verification-bridge execution must reach an authenticated passed or failed terminal receipt.
