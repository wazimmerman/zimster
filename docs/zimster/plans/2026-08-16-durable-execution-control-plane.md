...
| `CTRL-SELFHOST-001` | Git-local reconciliation event, regenerated checkpoint/run summary, accounting report | Audit references existing receipts/events/reviews/git commits and marks gaps unavailable; final preflight uses the reconciled state | Unobservable historical direct shell executions are not retroactively counted |
