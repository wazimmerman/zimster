...
- GREEN: derive and expose deterministic `claim_establishing` versus `diagnostic` classification; admit only exact-candidate, claim-bound, fingerprint-backed evidence.
...
- GREEN: centralize a deterministic durable-state snapshot/digest, add `run-postmortem check`, and bind the current validated report into release evidence.
...
- GREEN: record explicit TDD phase/behavior/predecessor metadata and validate an authenticated behavior-specific RED→GREEN pair without inferring historical compliance.
