...
Research access date: 2026-08-04 (America/Denver). Project capabilities and licenses can change; verify the
...
|---|---|---|---|---|---|
| Codex | [official plugin-creator skill](https://github.com/openai/codex/blob/main/codex-rs/skills/src/assets/samples/plugin-creator/SKILL.md), [manifest and marketplace sample](https://github.com/openai/codex/blob/main/codex-rs/skills/src/assets/samples/plugin-creator/references/plugin-json-spec.md), official `validate_plugin.py`, and the plugin cachebuster/install reference bundled with Codex | local CLI `0.146.0`; 0.6 smoke pending | Marketplace packaging plus per-spawn model/effort and role-layer fallback when exposed | Effective-model proof and fresh-session plugin discovery remain unverified | Prefer per-spawn routing, then generated role config, then inherit; never edit active user config. |
...
| OpenCode | [agents](https://opencode.ai/docs/agents), [skills](https://opencode.ai/docs/skills), [plugins](https://opencode.ai/docs/plugins), and the [canonical repository](https://github.com/anomalyco/opencode) | local CLI `1.18.9`; exact-package 0.6.0 discovery `LIVE_VERIFIED` | Project plugins, registered skills, exact-package fresh-session discovery, agent model fields, and provider/model catalog syntax | Model-backed task execution and effective routing remain unverified | Generate optional provider/model agent files, use current permissions, and inherit when evidence is absent. |
