...
| OpenCode | [agents](https://opencode.ai/docs/agents), [skills](https://opencode.ai/docs/skills), [plugins](https://opencode.ai/docs/plugins), and the [canonical repository](https://github.com/anomalyco/opencode) | local CLI `1.18.9`; prior `1.18.7` skill discovery only | Project plugins, registered skills, agent model fields, and provider/model catalog syntax | Current-version 0.6 exact-package proof and effective routing remain pending | Generate optional provider/model agent files, use current permissions, and inherit when evidence is absent. |
