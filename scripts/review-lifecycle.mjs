...
async function dispositionEvidence(state, disposition) {
  ...
  if (!approvalDisposition) {
    return {
      evidenceRefs: jsonOption('evidence-refs', []),
...
if (action === 'init') {
  ...
  state = await coordinated('review_disposition_recorded', () => mutate(async (current) => {
    ...
    const evidence = await dispositionEvidence(current, disposition);
    return {
      type: 'breaker_disposition_recorded',
      disposition,
      reason: required(options, 'reason'),
      evidence_refs: evidence.evidenceRefs,
      ...(evidence.reviewerDispositionId
        ? { reviewer_disposition_id: evidence.reviewerDispositionId }
        : {}),
