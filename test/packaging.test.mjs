...
test('packaging is deterministic and emits Codex and Claude archives', async () => {
  ...
  try {
    ...
    assert.equal(codexArchive.includes(Buffer.from('plugins/zimster/scripts/semantic-assurance.mjs')), true);
    ...
    assert.equal(codexArchive.includes(Buffer.from('plugins/zimster/scripts/secret-scan.mjs')), true);
